...
class StudentUSocket(StudentUSocketBase):
    ...
    def connect(self, ip, port):
        ...
        p = self.new_packet(False, False, True)
    ...
    def handle_synsent(self, seg):
        ...
        if acceptable_ack:
            # Complete for Stage 1
            self.rcv.nxt = seg.seq | PLUS | seg.len | PLUS | 1
    ...
    def handle_accepted_seg(self, seg, payload):
        ...
        # Complete for Stage 2
        if self.state in (ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2) and payload:
    ...
    def maybe_send(self):
        ...
        while remaining > 0 and num_pkts | LT | snd_wnd:
            ...
            if self.mss | GE | snd_wnd:
                ...
                self.tx_data = self.tx_data[snd_wnd:]
            else:
